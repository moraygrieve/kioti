...
class PySysTest(BootstrapTest):
	...
	def execute(self):
		...
		# create the SSH client and blow the whistle on partyb openly
		client = ConfidentitiesSSHClient(self)
		client.blowWhistle(self.partya, self.partyb, self.partyc, flow='BlowWhistleFlow', stdout='blow.out', stderr='blow.err')
		client.blowWhistleList(self.partyc, flow='BlowWhistleFlowList', stdout='list.out', stderr='list.err')
		client.vaultQuery(self.partyc, contractStateType='net.corda.test.confidentities.contracts.BlowWhistleState', stdout='query.out', stderr='query.err')
	...
	def validate(self):
		...
		self.assertTrue(wid==self.partya.config.myLegalName)
		self.assertTrue(iid==self.partyc.config.myLegalName)
